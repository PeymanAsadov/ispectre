...
const IpadProductCard = memo(({ item }) => {
  ...
  return (
    ...
    >
      <div>
        ...
        <p className="text-center text-xs text-gray-400 dark:text-[#A1A1AA] mt-1">
          {item.color}
        </p>
      </div>
...
IpadProductCard.displayName = "IpadProductCard";
...
function IpadCard({
  ...
  const cards = useMemo(() => {
    ...
    ipads.forEach((ipad) => {
      if (selectedModels.length && !selectedModels.includes(ipad.model)) return;
    ...
    return result;
  }, [ipads, minPrice, maxPrice, selectedModels, selectedColor, selectedStorage, sortBy]);
