...
import allIpads from "../../../data/ipad";
...
function IpadDetail() {
  ...
  const product = useMemo(
    () => allIpads.find((item) => String(item.id) === String(id)),
    [id]
  );
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f5f5f7] dark:bg-[#0f0f13] gap-4 px-4">
        ...
        <Link to="/ipad" className="text-blue-600 dark:text-blue-400 hover:underline">
  ...
  return (
    <div className="w-full min-h-screen bg-[#f5f5f7] dark:bg-[#0f0f13] text-[#1d1d1f] dark:text-gray-100 pb-16 animate-fade-in">
      <div className="max-w-7xl mx-auto px-4 md:px-6 pt-6 md:pt-12">
        ...
        {/* Breadcrumb - Mobil optimizasiyalı */}
        <div className="flex items-center gap-1.5 text-xs md:text-sm text-gray-500 dark:text-gray-400 mb-4 md:mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide">
          <Link to="/" className="text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white transition-colors duration-200">{t("nav.home")}</Link>
          <span>/</span>
          <Link to="/ipad" className="text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white transition-colors duration-200">iPad</Link>
          <span>/</span>
            ...
            <div className="w-full flex-1 flex flex-col md:flex-row gap-4 md:gap-8">
              ...
              <div className="flex-1 flex items-center justify-center min-h-[260px] md:min-h-[520px] order-1 md:order-2 bg-gray-50/50 dark:bg-black/5 rounded-2xl p-4">
                <img
                  ...
                  loading="eager"
                  decoding="async"
                  className="max-h-[240px] md:max-h-[520px] object-contain drop-shadow-[0_10px_15px_rgba(0,0,0,.05)] dark:drop-shadow-[0_20px_30px_rgba(0,0,0,.4)] hover:scale-105 transition duration-500"
                />
              </div>
            </div>
          ...
          >
            {suggested.map((item) => (
              <Link
                ...
                to={`/ipad/${item.id}?color=${encodeURIComponent(item.color)}`}
