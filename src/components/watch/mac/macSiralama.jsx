...
function MacSiralama({ title = "Bütün MacBook-lar", macbooks = [] }) {
  ...
  const allMemory = useMemo(() => {
    const set = new Set();
    allCards.forEach((card) => {
      card.storage.forEach((item) => set.add(item.size.trim()));
    });
    return [...set];
  }, [allCards]);
  ...
  const getModelCount = (model) =>
    allCards.filter((card) => card.model === model).length;

  const getColorCount = (color) =>
    allCards.filter((card) => card.color === color).length;
  ...
  const filteredCards = useMemo(() => {
    return allCards
      .map((card) => {
        const modelOk =
          selectedModel.length === 0 || selectedModel.includes(card.model);

        const colorOk =
          selectedColor.length === 0 || selectedColor.includes(card.color);
        ...
        const minPrice = Math.min(...validStorage.map((i) => Number(i.price)));
      ...
      .filter(Boolean);
  }, [allCards, selectedModel, selectedColor, selectedMemory, priceRange]);
            ...
            {allModels.map((model) => (
              <label key={model} className="flex items-center justify-between cursor-pointer">
                <div className="flex items-center">
                  <input type="checkbox" checked={selectedModel.includes(model)}
