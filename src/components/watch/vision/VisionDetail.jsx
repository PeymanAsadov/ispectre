...
function VisionDetail() {
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f5f5f7] dark:bg-[#0f0f13] gap-4 px-4">
        ...
        <Link to="/vision" className="text-blue-600 dark:text-blue-400 hover:underline font-medium">
  ...
  return (
    <div className="w-full min-h-screen bg-[#f5f5f7] dark:bg-[#0f0f13] text-[#1d1d1f] dark:text-gray-100 pb-16 animate-fade-in transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 md:px-6 pt-6 md:pt-12">
        ...
        <div className="flex items-center gap-1.5 text-xs md:text-sm text-gray-500 dark:text-gray-400 mb-4 md:mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide">
          ...
          <span>/</span>
          <Link to="/vision" className="hover:text-black dark:hover:text-white">Vision Pro</Link>
          <span>/</span>
          <span className="text-black dark:text-white font-medium truncate max-w-[150px]">{product.model}</span>
        </div>

        <div className="bg-white dark:bg-[#1c1c1e] rounded-3xl md:rounded-[40px] border border-gray-100 dark:border-gray-800 shadow-sm p-4 md:p-12 mb-8 md:mb-16">
          <div className="flex flex-col lg:flex-row gap-6 md:gap-12 items-start">
              ...
              <div className="flex md:flex-col gap-2 order-2 md:order-1 justify-center overflow-x-auto py-1">
                {color?.images?.map((image, index) => (
                  <button
                    ...
                    className={`w-14 h-14 md:w-24 md:h-24 flex-shrink-0 rounded-xl border overflow-hidden transition p-1 bg-white dark:bg-[#2c2c2e] ${selectedImageIdx === index ? "border-blue-500 ring-2 ring-blue-500/20" : "border-gray-200 dark:border-gray-700 hover:border-gray-400"}`}
                  >
            ...
            {/* Right : Information and Options */}
            <div className="w-full lg:w-96 flex-shrink-0">
              <h1 className="text-2xl md:text-4xl font-semibold tracking-tight text-gray-900 dark:text-white">{product.model}</h1>
              ...
              <div className="mt-4 md:mt-6 bg-[#f5f5f7] dark:bg-[#2c2c2e] rounded-2xl p-4 md:p-5 border border-gray-100 dark:border-transparent">
                <div className="flex items-baseline justify-between lg:block">
                  <h2 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">{price.toLocaleString()} ₼</h2>
                  <p className="text-[10px] md:text-xs text-gray-400 lg:mt-1">{t("product.payment_cash_card")}</p>
                </div>
                <div className="border-t border-gray-200 dark:border-gray-700 mt-3 pt-3 flex items-center justify-between lg:block">
                  ...
                  <p className="text-[10px] md:text-[11px] text-gray-400 hidden sm:block lg:block">{t("product.installments_info")}</p>
                </div>
              </div>

              {product.colors && product.colors.length > 1 && (
                <div className="mt-5 md:mt-8">
                  <h3 className="text-[11px] md:text-xs font-bold uppercase tracking-wider text-gray-400 mb-2 md:mb-3">Color</h3>
                  <div className="flex flex-wrap gap-1.5 md:gap-2">
                    {product.colors?.map((c) => (
                      <button
                        key={c.name}
                        onClick={() => handleColorChange(c)}
                        className={`px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm rounded-lg border transition font-medium ${color?.name === c.name
              ...
              <div className="mt-5 md:mt-8">
                <h3 className="text-[11px] md:text-xs font-bold uppercase tracking-wider text-gray-400 mb-2 md:mb-3">{t("product.storage")}</h3>
                <div className="grid grid-cols-2 gap-2">
                  {color?.storage?.map((item) => (
                    <button
                      key={item.size}
                      onClick={() => setVariant(item)}
                      className={`p-2.5 md:p-3 rounded-lg border text-left transition ${variant?.size === item.size
                    ...
                    >
                      <p className="font-semibold text-xs md:text-sm text-gray-900 dark:text-white">{item.size}</p>
                      <p className="text-[10px] md:text-xs text-gray-400 mt-0.5">{item.price} ₼</p>
                    </button>
                  ))}
                </div>
              </div>
              ...
              {specs.length > 0 && (
                <div className="mt-5 md:mt-8 border border-gray-200 dark:border-gray-700 rounded-2xl overflow-hidden">
                  <div className="bg-gray-50 dark:bg-[#2c2c2e] px-4 py-2.5 md:px-5 md:py-4 border-b border-gray-200 dark:border-gray-700">
                  ...
                  {specs.map((item, index) => (
                    <div key={index} className={`flex justify-between items-center px-4 py-2.5 md:px-5 md:py-4 text-xs md:text-sm bg-white dark:bg-[#1c1c1e] ${index !== specs.length - 1 ? "border-b border-gray-100 dark:border-gray-800" : ""}`}>
              ...
              <button
                ...
                disabled={added}
                className={`w-full mt-6 md:mt-8 py-3.5 md:py-4 rounded-xl md:rounded-2xl transition text-white font-semibold text-sm md:text-base shadow-sm ${added ? "bg-green-600" : "bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600"
                  }`}
              >
        ...
        {suggested.length > 0 && (
          <div className="mt-12 md:mt-16">
            <h2 className="text-xl md:text-2xl font-bold mb-4 md:mb-6 text-gray-900 dark:text-white">{t("product.you_may_also_like")}</h2>
            <div
              ref={sliderRef}
            ...
            >
              {suggested.map((item) => (
                <Link
                  ...
                  className="flex-shrink-0 w-[180px] md:w-[240px] bg-white dark:bg-[#1c1c1e] rounded-2xl md:rounded-[28px] border border-gray-100 dark:border-gray-800 p-4 md:p-6 hover:shadow-xl hover:-translate-y-1 md:hover:-translate-y-2 transition-all duration-300 group"
                >
                  <div className="w-full h-[120px] md:h-[180px] flex items-center justify-center mb-3 md:mb-4 overflow-hidden">
                  ...
                  <h3 className="text-xs md:text-sm font-semibold text-center text-gray-900 dark:text-white line-clamp-1">{item.model}</h3>
                  <p className="text-[10px] md:text-xs text-gray-500 dark:text-gray-400 text-center mt-0.5">{item.color}</p>
                  <p className="text-xs md:text-sm font-bold text-center mt-2 md:mt-3 text-gray-900 dark:text-white">
