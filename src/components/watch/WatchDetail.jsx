...
import allWatches from "../../data/watch";
...
function WatchDetail() {
  ...
  const product = useMemo(
    () => allWatches.find((item) => String(item.id) === String(id)),
    [id]
  );
  ...
  const specs = useMemo(() => {
    if (!product) return [];
    const list = [];
    if (product.chip) list.push({ name: t("product.chip") || "Çip", value: product.chip });
    if (product.display) list.push({ name: t("product.display") || "Ekran", value: product.display });
    if (variant?.size) list.push({ name: t("product.size") || "Ölçü", value: variant.size });
    if (color?.name) list.push({ name: t("product.color") || "Rəng", value: color.name });
    return list;
  }, [product, variant, color, t]);
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f5f5f7] dark:bg-[#0f0f13] gap-4 px-4">
        ...
        <Link to="/watch" className="text-blue-600 dark:text-blue-400 hover:underline font-medium">
    ...
    <div className="w-full min-h-screen bg-[#f5f5f7] dark:bg-[#0f0f13] text-[#1d1d1f] dark:text-gray-100 pb-16 animate-fade-in">
      <div className="max-w-7xl mx-auto px-4 md:px-6 pt-6 md:pt-12">
        ...
        <div className="flex items-center gap-1.5 text-xs md:text-sm text-gray-500 dark:text-gray-400 mb-4 md:mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide">
          ...
          <span>/</span>
          <Link to="/watch" className="hover:text-black dark:hover:text-white">Apple Watch</Link>
          <span>/</span>
            ...
            <div className="w-full lg:w-96 flex-shrink-0">
              ...
              <div className="mt-5 md:mt-8">
                ...
                <div className="flex flex-wrap gap-1.5 md:gap-2">
                  {[...new Map(product.colors?.map((c) => [c.name, c])).values()].map((c) => (
                    <button
                      key={c.name}
                      onClick={() => handleColorChange(c)}
                      className={`px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm rounded-lg border transition ${color?.name === c.name ? "bg-black text-white border-black dark:bg-slate-700 dark:border-slate-700" : "bg-white dark:bg-[#232326] border-gray-200 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500"}`}
                    >
                      {c.name}
                    </button>
                  ))}
              ...
              {color?.storage && color.storage.length > 1 && (
                <div className="mt-5 md:mt-8">
                  <h3 className="text-[11px] md:text-xs font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500 mb-2 md:mb-3">{t("product.size") || "Ölçü"}</h3>
