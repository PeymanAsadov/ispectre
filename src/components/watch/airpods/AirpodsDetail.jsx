...
import allAirpods from "../../../data/airpods";
...
function AirpodsDetail() {
  ...
  const product = useMemo(
    () => allAirpods.find((item) => String(item.id) === String(id)),
    [id]
  );
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f5f5f7] dark:bg-[#0f0f13] gap-4 px-4">
        ...
        <Link to="/airpods" className="text-blue-600 dark:text-blue-400 hover:underline font-medium">
    ...
    <div className="w-full min-h-screen bg-[#f5f5f7] dark:bg-[#0f0f13] text-[#1d1d1f] dark:text-gray-100 pb-16 animate-fade-in">
      <div className="max-w-7xl mx-auto px-4 md:px-6 pt-6 md:pt-12">
        ...
        <div className="flex items-center gap-1.5 text-xs md:text-sm text-gray-500 dark:text-gray-400 mb-4 md:mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide">
          ...
          <span>/</span>
          <Link to="/airpods" className="hover:text-black dark:hover:text-white">AirPods</Link>
          <span>/</span>
          ...
          <div className="flex flex-col lg:flex-row gap-6 md:gap-12 items-start">
            ...
            <div className="w-full lg:w-96 flex-shrink-0">
              ...
              <div className="mt-5 md:mt-8 border border-gray-200 dark:border-gray-700/50 rounded-2xl overflow-hidden">
                ...
                <div className="flex justify-between items-center px-4 py-2.5 md:px-5 md:py-4 text-xs md:text-sm border-b border-gray-100 dark:border-gray-700/50">
                  <span className="text-gray-500 dark:text-gray-400">{t("product.model_label") || "Model"}</span>
                  <span className="font-medium text-right">{product.model}</span>
                </div>
                <div className="flex justify-between items-center px-4 py-2.5 md:px-5 md:py-4 text-xs md:text-sm border-b border-gray-100 dark:border-gray-700/50">
                  <span className="text-gray-500 dark:text-gray-400">{t("product.color")}</span>
                  <span className="font-medium text-right">{color?.name}</span>
                </div>
                <div className="flex justify-between items-center px-4 py-2.5 md:px-5 md:py-4 text-xs md:text-sm">
                  <span className="text-gray-500 dark:text-gray-400">{t("product.storage")}</span>
                  <span className="font-medium text-right">{variant?.size || "Standart"}</span>
                </div>
              </div>
