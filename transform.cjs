...
function transformFile(filename, arrayName, transformFn, imports = '') {
  ...
  // Create temporary CommonJS file (.cjs extension is critical!)
  ...
  const tempFilename = 'temp_' + filename.replace(/\.js$/, '.cjs');
  const tempPath = path.join(srcDir, tempFilename);
...
transformFile('display.js', 'Display', (data) => {
  return data.map(item => {
    return {
      id: item.id,
      model: item.model,
      colors: colorHelper = item.colors.map(color => {
        return {
          name: color.name,
          images: getImagesArray(item, color),
          storage: (color.storage || []).map(v => ({
